...
%% dari path planner bertipe Circle
%% copyleft by Muhammad Fuad, Senin 16 Des 2019, @rumah Talon Madura

%% cara makai 
%  utk bentuk Circle
%  Belajar_Circle(0.01, 10, 5, 8,8) ==> utk full 1/4 circle
%  Belajar_Circle(0.01, 20, 5, 8,8) ==> utk full 2/4 circle
%  Belajar_Circle(0.01, 30, 5, 8,8) ==> utk full 3/4 circle
%  Belajar_Circle(0.01, 40, 5, 8,8) ==> utk full 4/4 circle

function [x,y] = Belajar_Circle(del_theta, k, r_path, XC, YC)
...
%% CLOVER LEAF == OK
% x = (amplitude)*sin(k/40*theta).*sin(2*k/40*theta);%pakai yg ini tidur, utk diskrit
% y = (amplitude)*cos(k/40*theta).*sin(2*k/40*theta);

%% CIRCLE == OK
x = (r_path)*cos(k/40*theta);%pakai yg ini tidur, utk diskrit
y = (r_path)*sin(k/40*theta);
...
% x = x + amplitude;
% y = y + amplitude;
f = figure('Visible', false);%% Rabu19Mei2021,14.19PM@F107
% plot(x,y,'k-');   
%xlim ylim perlu utk membatasi ukuran kurva CLOVER LEAF
% % xlim([0 amplitude]);
% % ylim([0 amplitude]);
xlim([0 r_path*3]);%pakai yg ini utk limit
ylim([0 r_path*3]);
delete(f);%% Rabu19Mei2021,14.19PM@F107