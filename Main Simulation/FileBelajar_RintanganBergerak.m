...
jmlObjek = 4;   %menentukan jumlah objek
lingkungan = MultiRobotEnv(jmlObjek);
lingkungan.robotRadius = [0.3 0.3 0.3 0.3];
...
y_square = [12, 12, 12.5, 12.5];
x_square1 = [6, 6.5, 6.5, 6];
y_square1 = [3, 3, 3.5, 3.5];
xx = [poses(1,3);poses(1,4)];
yy = [poses(2,3);poses(2,4)];
...
kontroler.DesiredLinearVelocity = 1;
...
kontroler1.DesiredLinearVelocity = 0.5;
...
kontroler2.DesiredLinearVelocity = 0.5;
...
for robotIdx = 1:jmlObjek    
        ...
        elseif robotIdx == 2 || robotIdx == 3 || robotIdx == 4
...
poses = [poses(:, 1), poses(:, 2), poses(:, 3), poses(:, 4)]; %ini dengan objek target
lingkungan([],[],poses,{},[]);
...
jarakKeObstacle = norm(poses(1:2,1) - poses(1:2,3));
...
dataJarakKeObstacle1DgnAmbang = ambangJarakAman/jarakKeObstacle;
...
dataJarakKeObstacle1 = jarakKeObstacle;
dataJarakKeObstacle2 = jarakKeObstacle;
%dataJarakKeObstacle3 = jarakKeObstacle;
...
while(jarakKeTarget > radiusTarget)
   ...
   for pedestrianIndek = 1:jmlObjek
       if pedestrianIndek == 1            
            ...
            statusBenturan = '';
            if jarakKeObstacle < 0.5 || jarakKeObstacle2 < 0.5
            ...
            dataJarakKeObstacle1DgnAmbang = [dataJarakKeObstacle1DgnAmbang  ambangJarakAman/jarakKeObstacle];
            ...
            dataJarakKeObstacle1 = [dataJarakKeObstacle1    jarakKeObstacle];
            ...
            str3 = sprintf('O#1|v:%0.2f,w:%0.2f,O#2|v:%0.2f,w:%0.2f',v2,w2,v3,w3); %manusiaTunggal            
            str4 = sprintf('DistTar:%0.2f|DistOb#1:%0.2f|DistOb#2:%0.2f', jarakKeTarget, jarakKeObstacle, jarakKeObstacle2);
   ...
   poses = [poses(:, 1), poses(:, 2), poses(:, 3), poses(:, 4)]; %ini dgn objek target
...
% PoseXObs = [dataPosisiXObstacle1; dataPosisiXObstacle2; dataPosisiXTarget]';           % Gabungan pose x obstacle
% PoseYObs = [dataPosisiYObstacle1; dataPosisiYObstacle2; dataPosisiYTarget]';           % Gabungan pose Y obstacle
% PoseORObs = [dataOrientasiObstacle1; dataOrientasiObstacle2; dataOrientasiTarget]';    % Gabungan orientasi obstacle
% %save("FilePoseRintanganBergerak.mat","PoseXObs","PoseYObs")
% save("FilePoseRB.mat","PoseXObs","PoseYObs","PoseORObs")
% %lidarPlot = cell2mat(RangesAll);
% save("FileBelajarRB.mat") %save data output lidar
% save("FileLidarRB.mat","scans")
