classdef Belajar_HVO2_Majemuk < matlab.System    
...
%        maxSpeed = 0.1;
    ...
    methods
        %konstruktor
        function obj = Belajar_HVO2_Majemuk(varargin)
        ...
        function [sudutHaluan] = menghitungHaluanBebasBenturan(obj,~)
            ...
            jarakAman = 0.3; %% lebih besar dari pada radius robot
            for indek=1:length(dataJarak)
                if indeknya(indek) > 3 && indeknya(indek) < 16
...
%                        dataJarak(indeknya(indek)+5) > jarakAman
        ...
        function [FV] = menyiapkanFV(obj,minVelo,maxVelo,kepadatan)
            ...
            for dataKe=1:jmlDataVelo
                ...
                jmlPengali(dataKe) = jmlData/kepadatan^dataKe;%misal: 8/(2^1) = 4                
            end %akhir for dataKe=1:jmlDataVelo
            jmlPengali = fliplr(jmlPengali); %[4;2;1]
            FV = zeros(jmlDataVelo, size(jmlDistribusi,2)^jmlDataVelo);%FV seukuran dgn 3 x dimensi jmlDistribusi
            for dataKe=1:jmlDataVelo %outer loop, misal: dari 1 s/d 3, sbg loop baris = 3
                ...
                for dataSisipanKe=1:kepadatan %inner loop, misal dari 1 s/d 2, sbg loop kolom = 2
                    iterasiDistribusi = repmat(jmlDistribusi(dataKe,dataSisipanKe),1,jmlPengali(dataKe));
                    dataYgDisisipkan = horzcat(dataYgDisisipkan, iterasiDistribusi);
                end %akhir for dataSisipanKe=1:kepadatan %inner loop
                jmlSalinan = jmlData/size(dataYgDisisipkan,2);
                FV(dataKe,:) = repmat(dataYgDisisipkan,1,jmlSalinan);
        ...
        function [kecepatanLinear, kecepatanAngular] = menghitungKecepatanBebasBenturan(obj,vel)
            ...
            kecepatanLinear = obj.himpunanAV;
            ...
            %rabu 05 feb 2020
            kecepatanAngular = obj.menghitungHaluanBebasBenturan(); %% Rabu12Mei2021
%             kecepatanAngular = haluan(1);            
        ...
        end %akhir function [HVO] = menghitungHVOtunggal(pr,vr,rr,po,vo,ro)
