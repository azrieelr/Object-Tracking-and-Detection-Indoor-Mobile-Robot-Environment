%% @F107,Selasa 15 Juni 2021, 15.21 PM
%% class MultiRobotEnv utk Skenario 5
classdef MultiRobotEnv_2AgenSilang < matlab.System
...
%         title = '';                 % edit judul visualisasi
        hasSedangMenghindar = false;% menerima perintah plot sedang hindar
    end
    properties
       % Lidar
    ...
    methods    
        ...
        function obj = MultiRobotEnv_2AgenSilang(N)
            obj.numRobots = N;
        end        
...
%         end  
    ...
    methods(Access = protected)
        ...
        function stepImpl(obj,robotIndices,poses,varargin)
            ...
            % tanda sedangMenghindar
            if any(obj.hasSedangMenghindar)
                obj.CA = varargin{idx};
            else
                obj.CA = [];%nanti diisi velos
            end
    ...
    methods (Access = public) 
        function sedangMenghindar(obj,aktifkanCA)
            obj.CA = aktifkanCA;
        end
        ...
        function setupVisualization(obj)
            ...
            % Create figure
            FigureName = 'Demo Penghindaran Rintangan by Muhammad Fuad';
            ...
            for rIdx = 1:obj.numRobots
                obj.OrientationHandle{rIdx} = plot(obj.ax,0,0,'r','LineWidth',3.5); %menggambar (plot) garis orientasi warna merah ('r') dengan ketebalan 1.5 pixel
...
% % %                                'Color','b','FontWeight','bold');
                    ...
                    if rIdx == 1
                            obj.IdHandles{rIdx} = text(0,0,'Robot', ... 
                               'Color','b','FontWeight','bold');                            
                    elseif rIdx == 2
                            obj.IdHandles{rIdx} = text(0,0,'Pursuer', ... 
                               'Color','b','FontWeight','bold');
                    elseif rIdx == 3
                            obj.IdHandles{rIdx} = text(0,0,'obstacle', ... 
                               'Color','b','FontWeight','bold');
                    elseif rIdx == 4
                            obj.IdHandles{rIdx} = text(0,0,'static', ... 
                               'Color','b','FontWeight','bold');
                    end
                end
            end
            ...
            % Initialize surface
            obj.SurfaceHandle = cell(obj.numRobots,1);
            ...
            for rIdx = 1:obj.numRobots
                if obj.showTrajectory(rIdx)
                    ...
                    if rIdx==1
                        ...
                        obj.SurfaceHandle{rIdx} = surface( ...
                          obj.ax, ...
                          'XData',[],...
                          'YData',[],...
                          'ZData',[], ...
...
%             title(obj.ax,'Visualisasi Multi-Robot');
            ...
            title(obj.ax,'Collision Avoidance Demo by Muhammad Fuad');
            hold(obj.ax,'off');
            axis equal
...
%                     'ydata',obj.trajY{rIdx});
                if rIdx == 1
                    set(obj.TrajHandle{rIdx},'xdata',obj.trajX{rIdx}, ...
                        'ydata',obj.trajY{rIdx}, 'color', [0 0 1]);
                    set(obj.SurfaceHandle{rIdx},'XData',[obj.trajX{rIdx} obj.trajX{rIdx}],...
                          'YData',[obj.trajY{rIdx} obj.trajY{rIdx}],...
                          'ZData',zeros(length(obj.trajY{rIdx}),2),...
                          'CData',[obj.CA(:) obj.CA(:)],...
                          'FaceColor','none',...
                          'EdgeColor','flat',...
                          'Marker','x');
                elseif rIdx == 2
                    set(obj.TrajHandle{rIdx},'xdata',obj.trajX{rIdx}, ...
                        'ydata',obj.trajY{rIdx}, 'color', [0 1 0]);
                end
            end
