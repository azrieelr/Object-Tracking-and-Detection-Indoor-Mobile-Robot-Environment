function [TrackList, betta_overall] = JPDAF_ENKF_Update(TrackList,DataList,ValidationMatrix, bettaNTFA)
...
for i=1:PointNum % Iterate over all measurements
    ...
    if (~isempty(temp_clust))   
        ...
        elseif (size(matched,2)>1) % If matched with more that one clusters
            ...
            % Start from last cluster, joining each one with the previous and removing the former.  
